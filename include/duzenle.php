        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"><?=$kontrol[0]["baslik"]?> Düzenleme Sayfası</li>
            </ol>
          </div>
        </div>
      </div>
          ...
          if($_POST)
          {
            if(!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"]))
            {
              ...
              if(!empty($_FILES["resim"]["name"]))
              {
                ...
                if($yukle!=false)
                {
                  $ekle = $VT->sorguCalistir("UPDATE ".$kontrol[0]["tablo"],"SET baslik=?, seflink=?, kategori=?, metin=?, resim=?, anahtar=?, description=?, durum=?, sirano=?, tarih=? WHERE id=?",array($baslik,$seflink,$kategori,$metin,$yukle,$anahtar,$description,1,$sirano,date("Y-m-d H:i:s"),$veri[0]["id"]));
                }
                else
                { ?>
              ...
              {
                  $ekle = $VT->sorguCalistir("UPDATE ".$kontrol[0]["tablo"],"SET baslik=?, seflink=?, kategori=?, metin=?, anahtar=?, description=?, durum=?, sirano=?, tarih=? WHERE id=?",array($baslik,$seflink,$kategori,$metin,$anahtar,$description,1,$sirano,date("Y-m-d H:i:s"),$veri[0]["id"]));
              }
              ...
              { ?>
                ...
                <meta http-equiv="refresh" content="5;url=<?=SITE?>liste/<?=$kontrol[0]["tablo"]?>">
                <?php
          ...
          <div class="card-body card card-default">
            <div class="row">
              ...
              <div class="col-md-12">
                <div class="form-group">
                  <label>Anahtar Kelimeler</label>
                    <input type="text" class="form-control" name="anahtar" placeholder="Anahtar Kelimeler ..." value="<?=stripslashes($veri[0]["anahtar"])?>">
                </div>
              </div>
              <div class="col-md-12">
                <div class="form-group">
              ...
              <div class="col-md-12">
                <div class="form-group">
                  <label>Sıra No</label>
                    <input type="number" class="form-control" name="sirano" placeholder="Sıra No ..." style="width: 120px;" value="<?=stripslashes($veri[0]["sirano"])?>">
                </div>
              </div>
              <div class="col-md-12">
                <div class="form-group">
